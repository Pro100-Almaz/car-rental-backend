...
class ForgotPassword:
    ...
    async def execute(self, request: ForgotPasswordRequest) -> None:
        ...
        if user is None:
            logger.info("Forgot password: email not found, returning silently.")
            return
        ...
        logger.debug("Password reset code for %s: %s", email.value, code)

        minutes = self._code_ttl.total_seconds() // 60
        await self._email_sender.send(
            to=email.value,
            subject="Password Reset Code",
            body=f"Your password reset code is: {code}\n\nThis code expires in {minutes:.0f} minutes.",
        )

        logger.info("Forgot password: done.")
