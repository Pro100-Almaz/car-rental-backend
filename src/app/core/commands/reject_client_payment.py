...
class RejectClientPaymentRequest:
    transaction_id: UUID
    rejection_reason: str


class RejectClientPayment:
    def __init__(
        self,
    ...
    async def execute(self, request: RejectClientPaymentRequest) -> None:
        logger.info("Reject client payment: started.")
        ...
        if transaction.status != TransactionStatus.PENDING:
            raise InvalidTransactionStatusTransitionError(
                f"Cannot reject transaction with status '{transaction.status}'."
            )
        ...
        logger.info("Reject client payment: done.")
