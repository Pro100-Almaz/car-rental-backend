...
class CheckinRental:
    ...
    async def execute(self, request: CheckinRentalRequest) -> None:
        logger.info("Checkin rental: started.")
        ...
        if rental.status != RentalStatus.CONFIRMED:
            raise InvalidRentalStatusTransitionError(f"Check-in requires status 'confirmed', got '{rental.status}'.")

        now = self._utc_timer.now.value
        rental.status = RentalStatus.ACTIVE
        rental.actual_start = now
