...
class BlacklistClient:
    ...
    async def execute(self, request: BlacklistClientRequest) -> None:
        logger.info("Blacklist client: started.")
        ...
        client.blacklist_reason = request.reason
        client.updated_at = UtcDatetime(self._utc_timer.now.value)
        await self._transaction_manager.commit()

        logger.info("Blacklist client: done.")
