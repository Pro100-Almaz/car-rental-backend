...
from app.core.commands.checkout_rental import CheckoutRental, CheckoutRentalRequest
...
class CheckoutRentalBody(BaseModel):
    model_config = ConfigDict(frozen=True)

    checkout_data: dict[str, Any]


def make_checkout_rental_router() -> APIRouter:
    ...
    async def checkout_rental(
        rental_id: UUID,
        body: CheckoutRentalBody,
        interactor: FromDishka[CheckoutRental],
    ) -> None:
        request = CheckoutRentalRequest(
            rental_id=rental_id,
            checkout_data=body.checkout_data,
        )
